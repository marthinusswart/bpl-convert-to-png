...
class AmigaBitplaneReader:
    ...
    def get_summary(self):
        ...
        return {
            ...
            "bytes_per_line": self.width // 8 if self.width > 0 else 0,
            ...
            for x_byte in range(width_bytes):
                ...
                for j in range(depth):
                    if self.interleaved:
                        ...
                        off = j * (width_bytes * height) + y * width_bytes + x_byte

                    color_plane_bytes.append(data[off] if off < len(data) else 0)

                # --- read mask byte (one per color plane when interleaved) ---
                # We only need one mask byte per x_byte position; use plane 0's
                # paired mask slot (they are all identical copies).
                mask_byte = None
                if self.has_mask:
                    if self.interleaved:
                        ...
                        off = (
                            y * depth * 2 * width_bytes
                            + 1 * width_bytes  # slot index 1 = mask after plane 0
                            + x_byte
                        )
                    else:
                        # Non-interleaved: mask is the last plane
                        off = depth * (width_bytes * height) + y * width_bytes + x_byte
